...
class TransformerMultiSrcDecoderLayer(nn.Module):
    ...
    def __init__(self, d_model, heads, d_ff, dropout, attention_dropout,
                 ...
                 alignment_heads=0):
        super(TransformerMultiSrcDecoderLayer, self).__init__()
        ...
        self.weight_template = nn.Linear(d_model, d_model)
        self.weight_source = nn.Linear(d_model, d_model)
    ...
    def _forward(self, inputs, memory_bank, src_pad_mask, tgt_pad_mask,
                 template_memory_bank, template_pad_mask, template_layer_cache,
...
class TransformerMultiSrcDecoder(DecoderBase):
    ...
    def __init__(self, num_layers, d_model, heads, d_ff,
                 ...
                 alignment_heads):
        super(TransformerMultiSrcDecoder, self).__init__()
        ...
        self.transformer_layers = nn.ModuleList(
            [TransformerMultiSrcDecoderLayer(d_model, heads, d_ff, dropout,
    ...
    def forward(self, tgt, memory_bank, step=None, **kwargs):
        ...
        cache_name_format = "template_layer_{}"
        if step == 0:
            self._init_cache(memory_bank[0])
            self._init_cache(memory_bank[1], cache_name_format=cache_name_format)
        ...
        src_memory_bank = memory_bank[0].transpose(0, 1).contiguous()
        template_memory_bank = memory_bank[1].transpose(0, 1).contiguous()
        ...
        for i, layer in enumerate(self.transformer_layers):
            ...
            template_layer_cache = self.state["cache"][cache_name_format.format(i)] \
                if step is not None else None
            output, attn, attn_align = layer(
                ...
                template_memory_bank=template_memory_bank,
                template_pad_mask=template_pad_mask,
                template_layer_cache=template_layer_cache
            )
    ...
    def _init_cache(self, memory_bank, cache_name_format="layer_{}"):
        ...
        for i, layer in enumerate(self.transformer_layers):
            ...
            self.state["cache"][cache_name_format.format(i)] = layer_cache
